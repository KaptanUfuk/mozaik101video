...
#OUT_FILE = 'mozaikframeler\\seniyazdimkalbime'+karesayisi+'.jpeg'
EOQ_VALUE = None
...
class TileProcessor:
	...
	def get_tiles(self):
		...
		print('{} adet kucuk resim islendi'.format(len(large_tiles)))
...
class TargetImage:
	...
	def get_data(self):
		print('Buyuk resmi goruyorum bekle')
...
class ProgressCounter:
	...
	def update(self):
		self.counter += 1
		print("%{:04.1f}".format(100 * self.counter / self.total), flush=True, end='\r')
...
def build_mosaic(result_queue, all_tile_data_large, original_img_large):
	...
	mosaic.save('mozaikframeler\\seniyazdimkalbime'&sys.argv[3]+3415&'.jpeg')
	#print('\nResim bitti su adla kaydettim', OUT_FILE)

def compose(original_img, tiles):
	print('Gorsel olusturuluyor lutfen bekleyin')
...
def mosaic(img_path, tiles_path):
	...
	if tiles_data[0]:
		...
		show_error("Buraya resim at kanka '{}'".format(tiles_path))

if __name__ == '__main__':
	if len(sys.argv) < 3:
		show_error('Kardes sen direk py mi calistiriyon oyle degil boyle kullancan : {} <resimyolu> <kucukresimlerinyolu>\r'.format(sys.argv[0]))
	else:
